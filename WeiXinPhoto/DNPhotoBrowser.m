...
- (void)checkButtonAction
{
    ...
    {
        if ([self.delegate respondsToSelector:@selector(ableToSelectImageWithPhotoBrowser:)]) {
            if ([self.delegate ableToSelectImageWithPhotoBrowser:self])
            {
...
- (UIButton *)checkButton
{
    if (nil == _checkButton) {
        ...
        _checkButton.frame = CGRectMake(0, 0, 16, 16);
        [_checkButton setBackgroundImage:[UIImage imageNamed:@"selected"] forState:UIControlStateSelected];
        [_checkButton setBackgroundImage:[UIImage imageNamed:@"notSelected"] forState:UIControlStateNormal];
...
- (DNFullImageButton *)fullImageButton
{
    ...
    [_fullImageButton setHidden:YES];
